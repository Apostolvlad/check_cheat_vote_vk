...
def checkError(r):
    ...
    if errorArr != None:
        ...
        input()
        return None
print("Введите адрес в строку браузера: https://oauth.vk.com/authorize?client_id=6270882&display=page&redirect_uri=https://oauth.vk.com/blank.html&scope=wall&v=5.92&response_type=token")
...
#https://oauth.vk.com/authorize?client_id=6270882&display=page&redirect_uri=https://oauth.vk.com/blank.html&scope=wall&response_type=token&v=5.92
vApi = "5.92"
try:
    result = requests.get('https://api.vk.com/method/polls.getById?poll_id=' + idPoll + "&owner_id=-" + idGroup + "&access_token=" + token + "&version=" + vApi).json() #, auth=('user', 'passwd')
    time.sleep(0.3)
...
for i in responseGetById["answers"]:
    ...
    while 1:
        try:
            result = requests.get('https://api.vk.com/method/polls.getVoters?count=500&poll_id=' + idPoll + "&offset=" + str(offsetUser) + "&answer_ids=" + str(id) + "&owner_id=" + idGroup + "&access_token=" + token + "&version=" + vApi).json() #, auth=('user', 'passwd')
            time.sleep(0.3)
        except requests.exceptions.ReadTimeout:
        ...
        try:
            result = requests.get('https://api.vk.com/method/groups.isMember?extended=1&group_id=' + idGroup + "&user_ids=" + userIds + "&access_token=" + token + "&version=" + vApi).json() #, auth=('user', 'passwd')
            time.sleep(0.3)
        except requests.exceptions.ReadTimeout:
